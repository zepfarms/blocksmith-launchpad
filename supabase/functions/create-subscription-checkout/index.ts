...
serve(async (req) => {
  ...
  try {
    ...
    const monthlyBlocks = pricing?.filter(p => 
      !p.is_free && 
      p.pricing_type === 'monthly' && 
      p.monthly_price_cents > 0
    ) || [];
    
    if (monthlyBlocks.length === 0) {
      return new Response(JSON.stringify({ error: 'No paid monthly blocks selected' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

    console.log('Creating subscription for blocks:', monthlyBlocks.map(b => b.block_name));

    // Create Stripe subscription checkout session
    const lineItems = monthlyBlocks.map((block, idx) => ({
      [`line_items[${idx}][price_data][currency]`]: 'usd',
      [`line_items[${idx}][price_data][unit_amount]`]: block.monthly_price_cents.toString(),
      [`line_items[${idx}][price_data][product_data][name]`]: `${block.block_name} (Monthly)`,
    ...
    const stripeResponse = await fetch('https://api.stripe.com/v1/checkout/sessions', {
      ...
      body: new URLSearchParams({
        ...
        'cancel_url': `${req.headers.get('origin')}/dashboard/app-store?subscription_cancelled=true`,

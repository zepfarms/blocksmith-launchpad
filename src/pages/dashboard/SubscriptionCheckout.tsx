...
export const SubscriptionCheckout = () => {
  ...
  const totalMonthlyCents = blocks.reduce((sum, block) => sum + block.monthly_price_cents, 0);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-neon-cyan to-neon-purple">
              Subscribe to Monthly Blocks
            </h1>
            <p className="text-muted-foreground">Start your monthly subscriptions</p>
          </div>
        </div>
            ...
            <Card className="p-6 border-white/10 sticky top-4">
              ...
              <div className="space-y-3 mb-6">
                <div className="flex justify-between text-sm">
                  ...
                  <span>${(totalMonthlyCents / 100).toFixed(2)}/mo</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Due Today</span>
                  <span>${(totalMonthlyCents / 100).toFixed(2)}</span>
                </div>
                <div className="border-t border-white/10 pt-3">
                  <div className="flex justify-between font-semibold text-lg">
                    <span>First Payment</span>
                    <span className="text-neon-purple">${(totalMonthlyCents / 100).toFixed(2)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-4 p-3 rounded-lg bg-blue-500/10 border border-blue-500/30">
                <p className="text-xs text-blue-400">
                  💡 You'll be charged ${(totalMonthlyCents / 100).toFixed(2)} every month. Cancel anytime from your dashboard.
                </p>
              </div>
              ...
              <p className="text-xs text-muted-foreground text-center mt-4">
                Secure recurring payments by Stripe
              </p>
            </Card>
          </div>
        </div>
